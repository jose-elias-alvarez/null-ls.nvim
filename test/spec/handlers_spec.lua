...
describe("handlers", function()
    ...
    describe("buf_request", function()
        ...
        it("should call original buf_request method with arguments", function()
            ...
            assert.stub(buf_request).was_called_with(1, "mockMethod", {}, mock_handler)
        end)

        it("should pass original handler when method matches but skip flag is set", function()
            handlers.buf_request(1, methods.lsp.CODE_ACTION, { _null_ls_skip = true }, mock_handler)

            assert.equals(buf_request.calls[1].refs[4], mock_handler)
        end)

        it("should pass wrapped handler when method matches and skip flag is not set", function()
            handlers.buf_request(1, methods.lsp.CODE_ACTION, {}, mock_handler)
    ...
    describe("buf_request_all", function()
        ...
        it("should set flag on params and pass arguments to original method", function()
    ...
    describe("setup_client", function()
        ...
        describe("notify", function()
            it("should call diagnostics handler with params", function()
                ...
                assert.stub(diagnostics.handler).was_called_with({ method = methods.lsp.DID_OPEN })
            end)
        ...
        describe("request", function()
            it("should return true and request_id if _null_ls_handled flag is set", function()
                local response, request_id = mock_client.request(
                    "mockMethod",
            ...
            it("should call original request handler if handled flag is not set", function()
                mock_request.returns(true)
                local response = mock_client.request("mockMethod", {}, mock_handler, 1)
        ...
        describe("cancel_request", function()
            it("should return true if request_id matches", function()
                local response = mock_client.cancel_request(methods.internal._REQUEST_ID)
