...
local write_good = {
    ...
    filetypes = {"markdown"},
    generators = {
        helpers.create_diagnostic_generator(
            {
                command = "write-good",
    ...
    generators = {
        {
            fn = function(params)
                ...
                local commentstring = api.nvim_buf_get_option(bufnr,
                                                              "commentstring")
                local raw_commentstring =
                    u.string.replace(commentstring, "%s", "")
                local line = params.content[params.row]

                local has_comment = string.find(line, raw_commentstring, nil,
                                                true)
                if has_comment then
                    return {
                        {
                            ...
                            action = function()
                                api.nvim_buf_set_lines(bufnr, params.row - 1,
                                                       params.row, false, {
                                    u.string
                                        .replace(line, raw_commentstring, "")
                                })
                            end
                        }
                    }
